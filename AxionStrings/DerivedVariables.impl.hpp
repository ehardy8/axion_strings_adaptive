                       ...
                       const int * /*bcomp*/, int /*scomp*/)
{
    ...
    amrex::ParmParse("wave").query("scalar_mass", scalar_mass);

    // The analytic solution assumes no potential; with a nonzero mass it is
    // left as zero (matches the KleinGordon example this was ported from).
    if (scalar_mass == 0)
    {
