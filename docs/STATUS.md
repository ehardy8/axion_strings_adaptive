...
- **Open (2026-09-27/28, with the user): the measured curvature distribution's
  ...
  **Possible contributor, flagged not pursued (2026-09-24)**: `interpolated_position`'s
  index-based point selection (consecutive pierced-face crossings, rather than
  points chosen at a fixed arc-length separation) has a baseline set by however
  densely the string happens to cross the lattice locally, not by physical
  distance -- verified on an exact circle to broaden the 5-95% spread of
  `kappa*R` from ~1% (arc-length-based, `l=4` lattice spacings) to ~35%
  (index-based), while biasing the *mean* by only ~0.004 dex (negligible next
  to the ~0.02-0.05 dex `hessian_analytic`/`interpolated_position` mean
  agreement already checked). So it does not explain that mean-level
  agreement, but a broadened, right-skewed distribution convolved with a
  narrower true one could plausibly smear and stabilise an apparent peak --
  worth a look if this puzzle is revisited, but not pursued now since nothing
  currently reported depends on the distribution's width or shape, only its
  mean/peak-adjacent statistics. Reported (independently, from a sister
  project) and independently re-verified here, same session as the
  `tangent_vector` fix above.
