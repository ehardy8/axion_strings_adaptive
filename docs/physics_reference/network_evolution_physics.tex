...
transforming or summing -- applied at exactly one place (filling the buffer handed to the FFT/reduction),
not inlined at several call sites, so an index or threshold error cannot silently diverge between them.
Three schemes:
\begin{itemize}
  \item \textbf{None} ($f=1$): unscreened pass, required for every screened/unscreened energy or spectrum
        pair (Sections~\ref{sec:energies},~\ref{sec:spectrum}) and for the tension's core+tail term.
  \item \textbf{A}: the bare numerator $\psivec_1\Pi_2-\Pi_1\psivec_2$, no division, no top-hat.
  \item \textbf{B}: the genuine phase rate $\theta'$ above, top-hat zeroed wherever $|\psivec|/R$ is below a
        runtime \texttt{threshold} (default $0.8$, not a settled choice -- $0.9$, $0.95$ have also been
        tried without reconciling on one value). The same $0/1$ top-hat is also the weight used to correct
        the effective point count in any spatial average over masked data (e.g.\ $n_{\rm unmasked}$ below).
\end{itemize}
